...
function main.center_text(text, reference, type, window_width, window_height, word_width)
    ...
    table.insert(vertically_centered_lines, "")
    local type_text = type == "quran" and "Quran" or "Hadith"
    table.insert(
        vertically_centered_lines,
        left_pad("- " .. reference .. " (" .. type_text .. ") -", (window_width - (string.len(reference) + string.len(type_text) + 7)) / 2)
    )
...
function main.get_verse(config)
    local index = (get_days() % #config.verses) + 1
    if config.mode == "random" then
        index = math.ceil(math.random() * #config.verses)
    end

    local verse = config.verses[index]
    return verse
end

function main.show_verse(config, window)
    local verse = main.get_verse(config)
    ...
    vim.api.nvim_buf_set_lines(
        ...
        0,
        false,
        main.center_text(verse.text, verse.verse, verse.type, window_width, window_height, text_width)
    )
